...
export class FormateursService {
  ...
  addFormateur(formateur: Formateur): void {
    this.formateurs = [...this.formateurs, formateur];
  }

  updateFormateur(updatedFormateur: Formateur): void {
    this.formateurs = this.formateurs.map((formateur) =>
      formateur.id === updatedFormateur.id ? updatedFormateur : formateur
    );
  }

  removeFormateur(id: number): void {
    this.formateurs = this.formateurs.filter((formateur) => formateur.id !== id);
  }
   getLastId(): number {
    return this.formateurs.length
      ? Math.max(...this.formateurs.map((formateur) => formateur.id))
      : 0;
  }
}